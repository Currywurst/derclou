...
{
    if (AskP
	(dbGetObject(gLandscapeState->ul_AreaID), ConsistOfRelationID, lso, NO_PARAMETER,
	 CMP_NO))
	return true;
...
void lsSetObjectRetrievalList(U32 ul_AreaId)
{
    ...
    for (i = 0; i < 3; i++)
	if (ul_AreaId == gLandscapeState->ul_ObjectRetrievalAreaId[i])
	    gLandscapeState->p_ObjectRetrieval = gLandscapeState->p_ObjectRetrievalLists[i];
}
...
U32 lsGetCurrObjectRetrieval(void)
{
    ...
    for (i = 0; i < 3; i++)
	if (gLandscapeState->p_ObjectRetrieval == gLandscapeState->p_ObjectRetrievalLists[i])
	    return gLandscapeState->ul_ObjectRetrievalAreaId[i];
...
{
    return ((uword) GetNrOfNodes(gLandscapeState->p_ObjectRetrieval));
}
...
ubyte lsGetLoudness(uword x, uword y)
{
	...
	LS_IS_MICRO_ON_FLOOR(gLandscapeState->p_CurrFloor[floorIndex].uch_FloorType))
	loudness = 15;
    else {
	for (i = -1; (i < 2) && (loudness == 255); i++) {
		...
		if ((k >= 0) && (k < LS_FLOORS_PER_LINE * LS_FLOORS_PER_COLUMN)) {
		    if ((!LS_NO_FLOOR(gLandscapeState->p_CurrFloor[k].uch_FloorType)) &&
			LS_IS_MICRO_ON_FLOOR(gLandscapeState->p_CurrFloor[k].uch_FloorType))
			loudness = 50;
		}
	    }
	}
...
U32 lsGetStartArea(void)
{
    ...
    startsWithAll(gLandscapeState->ul_BuildingID, OLF_NORMAL, Object_LSArea);
			 ...
			 ubyte showInvisible, ubyte addLootBags)
{
    ...
    for (node = (struct ObjectNode *) LIST_HEAD(gLandscapeState->p_ObjectRetrieval);
	 NODE_SUCC((NODE *) node);
...
{
    return (gLandscapeState->us_WindowXPos);
}

uword lsGetWindowYPos(void)
{
    return (gLandscapeState->us_WindowYPos);
}

uword lsGetTotalXPos(void)
{
    return (uword) (gLandscapeState->us_WindowXPos + gLandscapeState->us_PersonXPos);
}

uword lsGetTotalYPos(void)
{
    return (uword) (gLandscapeState->us_WindowYPos + gLandscapeState->us_PersonYPos);
}

void lsSetCollMode(ubyte collMode)
{
    gLandscapeState->uch_CollMode = collMode;
}

U32 lsGetCurrBuildingID(void)
{
    return gLandscapeState->ul_BuildingID;
}

U32 lsGetActivAreaID(void)
{
    return (gLandscapeState->ul_AreaID);
}
